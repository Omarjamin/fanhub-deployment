...
-- Post reporting system for posts moderation
--
...
CREATE TABLE `post_reports` (
  ...
  `reported_user_id` int(11) NOT NULL,
  `post_id` int(11) NOT NULL,
  `reason` enum('malicious photo','inappropriate picture') NOT NULL,
  `message_id` int(11) DEFAULT NULL,
  `status` enum('pending','reviewed','resolved','dismissed') DEFAULT 'pending',
  `admin_notes` text DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`report_id`),
  KEY `idx_post_report_reporter` (`reporter_id`),
  KEY `idx_post_report_reported_user` (`reported_user_id`),
  KEY `idx_post_report_post_id` (`post_id`),
  KEY `idx_post_report_created_at` (`created_at`),
  KEY `idx_post_report_status` (`status`),
  UNIQUE KEY `uq_reporter_post` (`reporter_id`,`post_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
ALTER TABLE `post_reports`
  ADD CONSTRAINT `fk_post_reports_reporter` FOREIGN KEY (`reporter_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_post_reports_reported_user` FOREIGN KEY (`reported_user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE;

--
-- Indexes for post reporting queries
--
CREATE INDEX `idx_post_report_counts` ON `post_reports` (`reported_user_id`, `created_at`);
CREATE INDEX `idx_post_unique_reporters` ON `post_reports` (`reported_user_id`, `reporter_id`, `created_at`);
